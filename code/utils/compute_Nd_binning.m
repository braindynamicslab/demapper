function [pts_in_bigBin, bin_indices, polygons] = compute_Nd_binning(filter_X, resolution, gain, nsides, display)
% Function bdl_binning_v1
...
if nsides == 3
    new_filter_min = filter_min * 1.1;
    new_filter_max = filter_max * 1.1;
    assert(all(new_filter_min < filter_min))
    assert(all(new_filter_max > filter_max))
    filter_min = new_filter_min;
    filter_max = new_filter_max;
end
...
for dim = 1:num_filters
    ...
    new_shape = mask_dim * (num_intervals(dim) - 1) + 1;
    dim_indices = repmat(reshape(1:num_intervals(dim), new_shape), new_size);
...
for bin = 1:num_bigBins 
    ...
    if num_filters ~= 2
        ...
        % scale and translate the polygon to its right place
